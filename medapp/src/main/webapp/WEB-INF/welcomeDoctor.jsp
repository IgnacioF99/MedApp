	...
	<div class="container-fluid d-flex flex-column flex-wrap">
		<header
			...
			<div  class="d-flex align-items-center">
				<a href="/logout" class="btn btn-custom me-2">Cerrar Sesión</a> <a
					href="/doctor/${userInSession.doctor2.id}" class="p-4"><img
		...
		<main>
			<div class="container">
							...
							<tbody>
								<c:forEach var="appointment" items="${appointments}">
									<tr>
										...
										<td><a
											href="/doctor/medicalHistory/${appointment.patient.id}">${appointment.patient.firstName}
												${appointment.patient.lastName}</a></td>
										<td>${appointment.appointmentInsurance}</td>
										<td>${appointment.patient.dni}</td>
										<td><a
											href="/doctor/medicalHistory/${appointment.patient.id}">Ver
												historial médico</a></td>
										<td><a
											href="/doctor/createMedicalHistory/${appointment.patient.id}">Generar
				...
				<div class="mb-5">
					<span><a href="/doctor/nextAppointments">Ver próximos
							turnos</a></span>
				</div>
			</div>
